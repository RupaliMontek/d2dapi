 ...
 //$query ="SELECT aa_dfia_licence_details.*, ship_bill_summary.sbs_id, ship_bill_summary.iec,item_details.invoice_id,item_details.item_id FROM aa_dfia_licence_details LEFT JOIN item_details ON aa_dfia_licence_details.item_id = item_details.item_id LEFT JOIN invoice_summary ON invoice_summary.invoice_id = item_details.invoice_id LEFT JOIN ship_bill_summary ON invoice_summary.sbs_id = ship_bill_summary.sbs_id  WHERE aa_dfia_licence_details.created_at >= NOW() - INTERVAL '24 HOURS'";
     ...
     //echo count($result);exit;
        foreach ($result as $str) {
            $iec = $str["iec"];
            $sql = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec'";
...
// Now you can use $database_name for your needs
echo "Current database name: " . $database_name;
        //$bm1 = "SELECT COUNT(*) FROM bill_of_entry_summary WHERE Where created_at >= current_date::timestamp and
         // created_at < current_date::timestamp + interval '1 day'";
        //$bm1 = "SELECT COUNT(*) FROM bill_of_entry_summary where iec_no = '0388004673'";
      echo  $bm1 = "SELECT COUNT(*) FROM bill_of_entry_summary";
        ...
        // Set the batch size
        $batchSize1 = 9000;
       //echo $result1_bm1[0]["count"];print_r($result1_bm1);exit;
        // Loop through the records in batches of 9000
        for ($offset1 = 0;$offset1 < $result1_bm1[0]["count"];$offset1 += $batchSize1) {
          //    for ($offset = 0;$offset < count($result_item_details);$offset += $batchSize) {
          //echo $offset1;
        $query1 = "SELECT * FROM bill_of_entry_summary ";
        $statement1 = $this->db->query($query1);
        $iecwise_data1 = [];
       ...
       // print_r($result1);exit;
        foreach ($result1 as $str1) 
         {
            // $iec1 = '0388004673';0888009364
            $iec1 =$str1["iec_no"];
            echo $sql = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec1'  ";
          /*  echo $sql = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' ";*/
...
/******************************checking dupliacte entries d2d**************************************/
       echo  $sql_beo = "SELECT * FROM bill_of_entry_summary WHERE bill_of_entry_summary.be_no = '$be_no'";
        $iecwise1_beo = $db1->query($sql_beo);
        $iecwise_data1_beo = array();
      // exit;
       /* while ($row = $iecwise1_beo->fetch_assoc()) {
          ...
          // $submission_date;
            $wbe_date = date("Y-m-d",strtotime($wbe_date));
              $submission_date = date("Y-m-d",strtotime($submission_date));
               ...
               $examination_date = date("Y-m-d",strtotime($examination_date));
            $ooc_date = date("Y-m-d",strtotime($ooc_date));
    //$dup_beo = $iecwise1_beo->result_array();
...
public function boe_delete_logs(){    $this->load->database('second');
        /******************************************************************Start bill_of_entry_summary***************************************************************************************/
    ...
    if(!empty($result_boe_delete_logs)){
        foreach ($result_boe_delete_logs as $str_boe_delete_logs) { //print_r($str_boe_delete_logs);
            ...
            $sql_boe_delete_logs = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_boe_delete_logs'";
...
public function bill_payment_details(){    $this->load->database('second');
        /******************************************************************Start bill_of_entry_summary***************************************************************************************/
//$query_bill_payment_details = "SELECT  bill_payment_details.* , bill_of_entry_summary.boe_id,bill_of_entry_summary.be_no,bill_of_entry_summary.iec_no FROM bill_payment_details  LEFT JOIN bill_of_entry_summary ON bill_of_entry_summary.boe_id = bill_payment_details.boe_id WHERE bill_payment_details.created_at >= CURRENT_DATE AND bill_payment_details.created_at >= NOW() - INTERVAL '24 HOURS'";
        ...
        $query_bill_payment_details =
            ...
            FROM bill_payment_details  LEFT JOIN bill_of_entry_summary ON bill_of_entry_summary.boe_id = bill_payment_details.boe_id";
   ...
   //echo count($result_bill_payment_details);exit;

        foreach ($result_bill_payment_details as $str_bill_payment_details) {
            $iec_bill_payment_details = $str_bill_payment_details["iec_no"];
             $sql_bill_payment_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_bill_payment_details'";
         ...
         /********************checking dupliacte entries d2d***********************/
        echo  $sql_users = "SELECT  bill_payment_details.* FROM bill_payment_details WHERE bill_payment_details.challan_no='$challan_no'";
                        $iecwise1_users = $db1_bill_payment_details->query($sql_users);
                        $iecwise_data1_users = array();
                  //      print_r($iecwise1_users);exit;
                        while ($rowusers = $iecwise1_users->fetch_assoc()) {
                        $iecwise_data1_users[] = $rowusers;
                        }
     ...
     /***********************************************************************/
         $dup_bill_payment_details = $iecwise1_users->result_array();
                       if(count($dup_bill_payment_details)== 0) {
...
$db1_bill_payment_details->close();
        $this->db->close();
        /******************************************************************End bill_of_entry_summary***************************************************************************************/
    }
...
public function inArray_bill_container_details($array, $value){
    ...
    // print_r($val);
             /* If value is found, set index to 1. */
     $c= $val['boe_id']."-".$val['container_number'];
             if($c == $value){
...
public function bill_container_details(){    $this->load->database('second');
        /******************************************************************Start bill_of_entry_summary***************************************************************************************/
$query_bill_container_details ="SELECT bill_container_details.*, bill_of_entry_summary.be_no,bill_of_entry_summary.boe_id,bill_of_entry_summary.iec_no FROM bill_container_details LEFT JOIN bill_of_entry_summary ON bill_of_entry_summary.boe_id = bill_container_details.boe_id";
...
//echo count($result_bill_container_details);exit;
        foreach (
            $result_bill_container_details
            as $str_bill_container_details
        ) {//print_r($str_bill_container_details);exit;
            $iec_bill_container_details = $str_bill_container_details["iec_no"];
            $sql_bill_container_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_bill_container_details'";
          ...
          /********************checking dupliacte entries d2d***********************/
                         $sql_users = "SELECT * FROM bill_container_details";
...
$db1_bill_container_details->close();
        $this->db->close();
        /******************************************************************End bill_of_entry_summary***************************************************************************************/
    }

public function bill_bond_details(){    $this->load->database('second');
    /******************************************************************Start bill_of_entry_summary***************************************************************************************/
        $query_bill_bond_details ="SELECT bill_bond_details.*, bill_of_entry_summary.boe_id,bill_of_entry_summary.be_no,bill_of_entry_summary.iec_no FROM 
        bill_bond_details LEFT JOIN bill_of_entry_summary ON bill_of_entry_summary.boe_id = bill_bond_details.boe_id";
        ...
        //echo  count($result_bill_bond_details);exit;
        //print_r($result_bill_bond_details);
        ...
        foreach ($result_bill_bond_details as $str_bill_bond_details) 
        {
           ...
           //$iec_bill_bond_details ='1308004672';
            $sql_bill_bond_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_bill_bond_details'";
       ...
       /********************checking dupliacte entries d2d***********************/
                     $sql_users = "SELECT bill_bond_details.*, bill_of_entry_summary.boe_id,bill_of_entry_summary.be_no,bill_of_entry_summary.iec_no 
...
public function bill_licence_details(){    $this->load->database('second');
        /******************************************************************Start bill_of_entry_summary***************************************************************************************/
       $query_bill_licence_details =
       "SELECT CONCAT(n1.be_no,'-',bill_licence_details.invsno,'-',bill_licence_details.itemsn) as reference_code,n1.boe_id, n1.iec_no,n1.be_no,n1.be_date, bill_licence_details.* FROM 
       (select * from bill_of_entry_summary order by boe_id ) n1 JOIN invoice_and_valuation_details ON n1.boe_id = invoice_and_valuation_details.boe_id 
       ...
       JOIN bill_licence_details ON duties_and_additional_details.duties_id = bill_licence_details.duties_id";
        ...
        foreach ($result_bill_licence_details as $str_bill_licence_details) {
            ...
            $sql_bill_licence_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_bill_licence_details'";
...
$db1_bill_licence_details->close();
        $this->db->close();
}
        /******************************************************************End bill_of_entry_summary***************************************************************************************/
    }
...
public function boe_file_status(){    $this->load->database('second');
    /******************************************************************Start bill_of_entry_summary***************************************************************************************/
  ...
  // print_r($result_boe_file_status);//exit;
    ...
    foreach ($result_boe_file_status as $str_boe_file_status) {
        $iec_boe_file_status = $str_boe_file_status["user_iec_no"];
         $sql_boe_file_status = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_boe_file_status%'";
...
public function bill_manifest_details(){    $this->load->database('second');
    /******************************************************************Start bill_of_entry_summary***************************************************************************************/
   // $query_bill_manifest_details ="SELECT * from bill_manifest_details";
...
//       $query_bill_manifest_details ="SELECT bill_manifest_details.*, bill_of_entry_summary.boe_id,bill_of_entry_summary.be_no,bill_of_entry_summary.iec_no FROM bill_manifest_details LEFT JOIN bill_of_entry_summary ON bill_of_entry_summary.boe_id = bill_manifest_details.boe_id where bill_of_entry_summary.iec_no='3194011511'";
    ...
    //echo count($result_bill_manifest_details);exit;
    foreach ($result_bill_manifest_details as $str_bill_manifest_details) {
     ...
     //$iec_bill_manifest_details ='3194011511';
        ...
        $db1_bill_manifest_details = $this->database_connection(
            ...
            /********************checking dupliacte entries d2d***********************/
            $sql_users ="SELECT bill_manifest_details.*, bill_of_entry_summary.boe_id,bill_of_entry_summary.be_no,bill_of_entry_summary.iec_no 
            FROM bill_manifest_details LEFT JOIN bill_of_entry_summary ON bill_of_entry_summary.boe_id = bill_manifest_details.boe_id 
            WHERE bill_of_entry_summary.be_no=".$be_no." AND bill_manifest_detailsigm_no=".$igm_no;
 ...
 /***********************************************************************/
       ...
       echo  $sql_insert_bill_manifest_details =
            "INSERT INTO `bill_manifest_details` (`boe_id`, `igm_no`, `igm_date`, `inw_date`, `gigmno`, `gigmdt`,`mawb_no`,`mawb_date`,`hawb_no`,`hawb_date`,`pkg`,`gw`,`created_at`) VALUES('" .
            $boe_id .
            "','" .
            $igm_no .
            "','" .
...
}$db1_bill_manifest_details->close();
        $this->db->close();
    /******************************************************************End bill_of_entry_summary***************************************************************************************/
}
...
public function challan_details(){    $this->load->database('second');
    /******************************************************************Start bill_of_entry_summary***************************************************************************************/
    $query_challan_details ="SELECT challan_details.*,ship_bill_summary.sbs_id,ship_bill_summary.sb_no,ship_bill_summary.iec FROM challan_details  JOIN ship_bill_summary ON ship_bill_summary.sbs_id=challan_details.sbs_id";
   ...
   // $query_challan_details ="SELECT challan_details.*,ship_bill_summary.sbs_id,ship_bill_summary.sb_no,ship_bill_summary.iec FROM challan_details  JOIN ship_bill_summary ON ship_bill_summary.sbs_id=challan_details.sbs_id  where ship_bill_summary.iec='0388004673'";
    ...
    foreach ($result_challan_details as $str_challan_details) {//print_r($str_challan_details);
        ...
        $sql_challan_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_challan_details'";
        //$sql_challan_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '0388004673'";
        ...
        //if (get_magic_quotes_gpc()) {
          $challan_id = addslashes($str_challan_details["challan_id"]);
           ...
           /********************checking dupliacte entries d2d***********************/
     $sql_users ="SELECT challan_details.*,ship_bill_summary.sbs_id,ship_bill_summary.sb_no,ship_bill_summary.iec 
     FROM challan_details  JOIN ship_bill_summary ON ship_bill_summary.sbs_id=challan_details.sbs_id where ship_bill_summary.sbs_id='".$sbs_id."'";
                    ...
                    $iecwise1_users1111111 = $db1_challan_details->query($sql_users1111);
                 
     
echo $iecwise1_users1111111->num_rows;
...
public function drawback_details(){    $this->load->database('second');
    /******************************************************************Start bill_of_entry_summary***************************************************************************************/
...
//$query_drawback_details ="SELECT CONCAT(n1.sb_no,'-',invoice_summary.s_no_inv, '-', item_details.item_s_no) as reference_code, sb_no, sb_date, iec_br,iec, inv_sno, item_sno, item_details.hs_cd, item_details.description, dbk_sno, qty_wt, value, dbk_amt, stalev, cenlev, drawback_details.* FROM (select * from ship_bill_summary order by sbs_id desc) n1 JOIN invoice_summary ON invoice_summary.sbs_id = n1.sbs_id JOIN item_details ON invoice_summary.invoice_id = item_details.invoice_id JOIN drawback_details ON drawback_details.item_id = item_details.item_id";
    ...
    foreach ($result_drawback_details as $str_drawback_details) {
       ...
       // $sql_drawback_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_drawback_details'";0388004673
                $sql_drawback_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_drawback_details'";
...
$db1_drawback_details->close();
        $this->db->close();
    /******************************************************************End bill_of_entry_summary***************************************************************************************/
}


public function cb_file_status(){    $this->load->database('second');
        /******************************************************************Start bill_of_entry_summary***************************************************************************************/
       ...
       // print_r($result_cb_file_status);exit;
        ...
        foreach ($result_cb_file_status as $str_cb_file_status) {
            ...
            $sql_cb_file_status = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_cb_file_status'";
...
$db1_cb_file_status->close();
        $this->db->close();
        /******************************************************************End bill_of_entry_summary***************************************************************************************/
    }

public function courier_bill_bond_details(){    $this->load->database('second');
        /******************************************************************Start bill_of_entry_summary***************************************************************************************/
       ...
       // print_r($result_courier_bill_bond_details);exit;
        ...
        foreach (
            ...
            $sql_courier_bill_bond_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_courier_bill_bond_details'";
...
$db1_courier_bill_bond_details->close();
        $this->db->close();
        /******************************************************************End bill_of_entry_summary***************************************************************************************/
    }

public function courier_bill_container_details(){    $this->load->database('second');
     ...
     //   print_r($result_courier_bill_container_details);exit;
        ...
        foreach (
            ...
            $sql_courier_bill_container_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_courier_bill_container_details'";
...
$db1_insert_courier_bill_container_details->close();
        $this->db->close();
        /******************************************************************End courier_bill_container_details***************************************************************************************/
    }

public function courier_bill_duty_details(){    $this->load->database('second');
     ...
     //   print_r($result_courier_bill_duty_details);exit;
        ...
        foreach (
            ...
            $sql_courier_bill_duty_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_courier_bill_duty_details'";
...
$db1_courier_bill_duty_details->close();
        $this->db->close();
        /******************************************************************End bill_of_entry_summary***************************************************************************************/
    }

public function courier_bill_igm_details(){    $this->load->database('second');
        /******************************************************************Start courier_bill_container_details***************************************************************************************/
        ...
        foreach (
            ...
            $sql_courier_bill_igm_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_courier_bill_igm_details'";
...
public function courier_bill_invoice_details(){    $this->load->database('second');
        /******************************************************************Start courier_bill_container_details***************************************************************************************/
      ...
      //  print_r($result_courier_bill_invoice_details);exit;
        ...
        foreach (
            ...
            $sql_courier_bill_invoice_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_courier_bill_invoice_details'";
...
$db1_courier_bill_invoice_details->close();
        $this->db->close();
        /******************************************************************End courier_bill_container_details***************************************************************************************/
    }
public function courier_bill_items_details(){    $this->load->database('second');
       ...
       // print_r($result_courier_bill_items_details);exit;
        ...
        foreach (
            ...
            $sql_courier_bill_items_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_courier_bill_items_details'";
...
'" .
 ...
 $db1_courier_bill_items_details->close();
        $this->db->close();   
    
    
}

public function courier_bill_manifest_details(){    $this->load->database('second');
        /******************************************************************Start courier_bill_container_details***************************************************************************************/
...
VALUES('" .
 ...
 $db1_courier_bill_manifest_details->close();
        $this->db->close(); 
        /******************************************************************End courier_bill_container_details***************************************************************************************/
    }

public function courier_bill_notification_used_for_items(){    $this->load->database('second');
        /******************************************************************Start courier_bill_container_details***************************************************************************************/
...
$db1_courier_bill_notification_used_for_items->close();
        $this->db->close(); 
        /******************************************************************End courier_bill_container_details***************************************************************************************/
    }

public function courier_bill_payment_details(){    $this->load->database('second');
...
public function courier_bill_procurment_details(){    $this->load->database('second');
...
public function courier_bill_summary(){    $this->load->database('second');
       ...
       // print_r($result_courier_bill_summary);exit;
        ...
        foreach ($result_courier_bill_summary as $str_courier_bill_summary) {
            ...
            $sql_courier_bill_summary = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_courier_bill_summary'";
...
public function item_details(){    $this->load->database('second');
        ...
        JOIN ship_bill_summary ON ship_bill_summary.sbs_id=invoice_summary.sbs_id "; 
        $statement_item_details = $this->db->query($query_item_details);
        $iecwise_item_details = [];
        $result_item_details = $statement_item_details->result_array();
       //echo count($result_item_details);exit;
        $batchSize = 9000;
...
/***********************************************************************/    
   if ($iecwise1_usersssss->num_rows == 0) {
...
public function invoice_and_valuation_details(){    $this->load->database('second');
    
    
    /*echo $q= "SELECT * from  invoice_and_valuation_details where boe_id='137198'";
            $s = $this->db->query($q);
        $r= $s->result_array();
 print_r($r);exit;*/
   /*   echo  $query_invoice_and_valuation_details ="SELECT invoice_and_valuation_details.*,bill_of_entry_summary.iec_no 
            FROM invoice_and_valuation_details LEFT JOIN bill_of_entry_summary ON bill_of_entry_summary.boe_id = invoice_and_valuation_details.boe_id
            where  invoice_and_valuation_details.boe_id::text  LIKE '140627'  AND bill_of_entry_summary.iec_no LIKE '0888009364'";
   */         
               $query_invoice_and_valuation_details =
            "SELECT invoice_and_valuation_details.*,bill_of_entry_summary.iec_no 
...
//echo  count($result_invoice_and_valuation_details);exit;
 // print_r($result_invoice_and_valuation_details);
$batchSize = 1000;
      ...
      $sql_invoice_and_valuation_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin'  AND iec_no LIKE '%$iec_invoice_and_valuation_details'";
           //$sql_invoice_and_valuation_details = "SELECT lucrative_users_id,iec_no  FROM lucrative_users where role='admin'";

           //$sql_invoice_and_valuation_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' ";
           ...
           $db1_invoice_and_valuation_details = $this->database_connection($iecwise_data_invoice_and_valuation_details[0]["lucrative_users_id"]);
...
/********************checking dupliacte entries d2d***********************/
     echo $sql_users ="SELECT invoice_and_valuation_details.*, bill_of_entry_summary.boe_id,bill_of_entry_summary.iec_no 
      FROM invoice_and_valuation_details 
      LEFT JOIN bill_of_entry_summary ON bill_of_entry_summary.boe_id = invoice_and_valuation_details.boe_id 
      WHERE invoice_and_valuation_details.s_no = '$s_no' and invoice_and_valuation_details.invoice_no = '$invoice_no' 
      and invoice_and_valuation_details.invoice_date='$invoice_date'";
 /*echo $sql_users ="SELECT invoice_and_valuation_details.*  FROM invoice_and_valuation_details 
      WHERE invoice_and_valuation_details.s_no = '$s_no' and invoice_and_valuation_details.invoice_no = '$invoice_no' 
      and invoice_and_valuation_details.invoice_date='$invoice_date'";*/

     /*echo $sql_users ="SELECT invoice_and_valuation_details.*  FROM invoice_and_valuation_details";*/
                $iecwise_data1_users = $db1_invoice_and_valuation_details->query($sql_users);
    // echo $iecwise_data1_users->num_rows;exit;        
                /* while ($rowusers = $iecwise1_users->fetch_assoc()) {
                $iecwise_data1_users[] = $rowusers;
                }
 ...
 /***********************************************************************/
 if ($iecwise_data1_users->num_rows == 0) {       
...
public function invoice_and_valuation_details_ORIGENAL(){    $this->load->database('second');
        $query_invoice_and_valuation_details =
...
public function rodtep_details(){    $this->load->database('second');
...
// $query_rodtep_details ="SELECT CONCAT(ship_bill_summary.sb_no,'-',invoice_summary.s_no_inv, '-', item_details.item_s_no) as reference_code,rodtep_details.*,item_details.invoice_id,invoice_summary.invoice_id,invoice_summary.sbs_id,ship_bill_summary.iec,ship_bill_summary.sbs_id,ship_bill_summary.sb_date FROM rodtep_details LEFT JOIN item_details ON item_details.item_id=rodtep_details.item_id LEFT JOIN invoice_summary ON invoice_summary.invoice_id=item_details.invoice_id LEFT JOIN ship_bill_summary ON ship_bill_summary.sbs_id=invoice_summary.sbs_id WHERE item_details.created_at >= NOW() - INTERVAL '24 HOURS'";
 ...
 $query_rodtep_details =  "SELECT CONCAT(ship_bill_summary.sb_no,'-',rodtep_details.inv_sno, '-', rodtep_details.item_sno) as reference_code,rodtep_details.*,item_details.invoice_id,invoice_summary.invoice_id,invoice_summary.sbs_id,ship_bill_summary.iec,ship_bill_summary.sbs_id,ship_bill_summary.sb_date FROM rodtep_details LEFT JOIN item_details ON item_details.item_id=rodtep_details.item_id LEFT JOIN invoice_summary ON invoice_summary.invoice_id=item_details.invoice_id LEFT JOIN ship_bill_summary ON ship_bill_summary.sbs_id=invoice_summary.sbs_id  ";
       ...
       // count($result_rodtep_details);exit;
...
$batchSize = 9000;
        ...
        foreach ($result_rodtep_details as $str_rodtep_details) {
            ...
            $sql_rodtep_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_rodtep_details'";
...
public function jobbing_details(){    $this->load->database('second');
        /******************************************************************Start jobbing_details***************************************************************************************/

        $query_jobbing_details =
        ...
        foreach ($result_jobbing_details as $str_jobbing_details) {
            ...
            $sql_jobbing_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_jobbing_details'";
...
$sql_users =
 ...
 '" .
     ...
     $db1_insert_jobbing_details->close();
        $this->db->close();
...
public function ship_bill_summary()
 {
		          ...
		          $query_ship_bill_summary
		           ...
		           //echo count($result_ship_bill_summary);exit;
		         foreach ($result_ship_bill_summary as $str_ship_bill_summary) 
		         {
		         	$iec_ship_bill_summary = $str_ship_bill_summary["iec"];
		         	$sql_ship_bill_summary = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_ship_bill_summary'";
                ...
                $iecwise_data1_users = array();
            // echo $iecwise1_users->num_rows;exit;
                /*    while ($rowusers = $iecwise1_users->fetch_assoc()) {
                $iecwise_data1_users[] = $rowusers;
                }
                $c= $sb_no."-".$sb_date;
...
public function  duties_and_additional_details_cron()
{
     $status =  $this->database_connection_master();
     $query ="select * from lucrative_users  ";
     $lucrtaive_users = $this->db->query($query);
     $lucrtaive_userss = $lucrtaive_users->result_array(); 
     foreach($lucrtaive_userss as $lucrative_user)
     {
        $iec_no= $lucrative_user["iec_no"];
        ...
        ON invoice_and_valuation_details.invoice_id = duties_and_additional_details.invoice_id  where  iec_no = '$iec_no' ";
         $statement_duties_and_additional_details = $this->db->query(
            $query_duties_and_additional_details
        );
        $iecwise_duties_and_additional_details = [];
        ...
        foreach (
          ...
          /* */ $sql_duties_and_additional_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_no%'";
        ...
        LEFT JOIN invoice_and_valuation_details
                ...
                $iecwise_data1_users = array();
                //print_r($iecwise_data1_users); die();
...
$copy_insert_duties_and_additional_details = $db1_duties_and_additional_details->query(
   ...
   $db1_duties_and_additional_details->close();
        $this->db->close(); 
}
    
public function duties_and_additional_details(){    $this->load->database('second');
        /******************************************************************Start duties_and_additional_details***************************************************************************************/
   ...
   //  echo  count($result_duties_and_additional_details);exit;
        ...
        // Loop through the records in batches of 9000
        for ($offset = 0;$offset < count($result_duties_and_additional_details);$offset += $batchSize ) {
     ...
     //   echo $sql_duties_and_additional_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin'  AND iec_no LIKE '3194011511'";
       ...
       where  CONCAT(bill_of_entry_summary.be_no,'-',invoice_and_valuation_details.s_no, '-', duties_and_additional_details.s_no)='$reference_code' AND bill_of_entry_summary.be_no='$be_no' AND bill_of_entry_summary.be_date='$be_date'";
...
public function equipment_details(){    $this->load->database('second');
    
        /******************************************************************Start equipment_details***************************************************************************************/
  $query_equipment_details ="SELECT equipment_details.*,ship_bill_summary.sbs_id,ship_bill_summary.sb_no,ship_bill_summary.iec FROM equipment_details LEFT JOIN ship_bill_summary ON ship_bill_summary.sbs_id=equipment_details.sbs_id  where ship_bill_summary.iec='0388081627' ";
 ...
 // $query_equipment_details ="SELECT equipment_details.*,ship_bill_summary.sbs_id,ship_bill_summary.sb_no,ship_bill_summary.iec FROM equipment_details LEFT JOIN ship_bill_summary ON ship_bill_summary.sbs_id=equipment_details.sbs_id ";
        ...
        foreach ($result_equipment_details as $str_equipment_details) 
        {
                ...
                $sql_equipment_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_equipment_details'";
            ...
            /*************************************checking dupliacte entries d2d************************************************/
                echo  $sql_users = "SELECT equipment_details.*,ship_bill_summary.sbs_id,ship_bill_summary.sb_no,ship_bill_summary.iec FROM equipment_details LEFT JOIN ship_bill_summary ON ship_bill_summary.sbs_id=equipment_details.sbs_id ";
...
public function item_manufacturer_details(){    $this->load->database('second');
        ...
        foreach (
            ...
            $sql_item_manufacturer_details = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_item_manufacturer_details'";
...
VALUES('" .
 ...
 $db1_insert_item_manufacturer_details->close();
        $this->db->close(); 
        /******************************************************************Start item_manufacturer_details***************************************************************************************/
    }
...
/*public function invoice_summery(){    $this->load->database('second');
...
//  $query_invoice_summery ="SELECT invoice_summary.*,ship_bill_summary.iec,ship_bill_summary.sbs_id FROM invoice_summary JOIN ship_bill_summary ON invoice_summary.sbs_id=ship_bill_summary.sbs_id";
        ...
        foreach ($result_invoice_summery as $str_invoice_summery) {
            ...
            $sql_invoice_summery = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_invoice_summery'";
...
VALUES('" .
        ...
        /******************************************************************Start invoice_summery***************************************************************************************/
$db1_insert_invoice_summery->close();
        $this->db->close();  
        
    
}  
/*SELECT CONCAT(n1.sb_no,'-',invoice_summary.s_no_inv, '-', item_details.item_s_no) as reference_code, 
       n1.sb_no, n1.sb_date, n1.iec_br, n1.inv_sno, item_details.item_sno, n1.iec_tpd, n1.exporter_name, 
       third_party_details.address, third_party_details.gstn_id_type
FROM (SELECT * FROM ship_bill_summary ORDER BY sbs_id DESC LIMIT ? OFFSET ?) n1
...
public function sb_file_status(){    $this->load->database('second');
        /******************************************************************Start sb_file_status***************************************************************************************/

         $query_sb_file_status = "SELECT * FROM sb_file_status ";
       ...
       // print_r($result_sb_file_status);exit;
        ...
        foreach ($result_sb_file_status as $str_sb_file_status) {
            ...
            $sql_sb_file_status = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_sb_file_status'";
     ...
     $this->load->database('second');
      ...
      ORDER BY n1.sbs_id DESC";
       ...
       // print_r($result_sb_file_status);exit;
        ...
        foreach ($result_sb_file_status as $str_sb_file_status) {
            $iec_sb_file_status = $str_sb_file_status["iec"];
            $sql_sb_file_status = "SELECT lucrative_users_id  FROM lucrative_users where role='admin' AND iec_no LIKE '%$iec_sb_file_status'";
            $iecwise_sb_file_status = $this->db->query($sql_sb_file_status);
            $iecwise_data_sb_file_status = $iecwise_sb_file_status->result_array();
            $db1_sb_file_status = $this->database_connection(
...
ORDER BY n1.sbs_id DESC ";
                ...
                $iecwise_data1_users[] = $rowusers;
                   ...
                   $c= $reference_code;
                    //skip dupliacte entry     
             $a= $this->inArray_third_party_details($iecwise_data1_users,$c); // Output - value exists
             if ($a==1) {
                   "Duplicate";"============";continue;
             }
...
public function database_connection($id){
        ...
        $username = "root";
        $password = "%!#^bFjB)z8C";
        $db_name1 = "lucrativeesystem_D2D_".$id;
    ...
    public function database_connection_master(){    $this->load->database('second');
        $hostname = "localhost";
        $username = "root";
        $password = "%!#^bFjB)z8C";
...
public function saveimport_export_data(){    $this->load->database('second');
        $post = $this->input->post();
    }

public function iec_signup(){    $this->load->database('second');
...
public function register_iec_user_old(){    $this->load->database('second');
        $post = $this->input->post();
        $alphabet =
            "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890";
        $pass = []; //remember to declare $pass as an array
...
public function get_worksheet_name_by_type(){    $this->load->database('second');
        $post = $this->input->post();
...
public function get_worksheet_columns(){    $this->load->database('second');
        $data = "";
        $user_id = $_SESSION["user_id"];
        $iec_id = $_SESSION["iec_no"];
        $db_name = $_SESSION["database_prefix"] . $user_id;
