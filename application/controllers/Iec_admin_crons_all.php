...
public function bill_of_entry_summary()
{
    ...
    for ($page = 0; $page < $pages; $page++) {
     ...
     // echo  $bill_of_entry_summary_query = "SELECT * FROM bill_of_entry_summary WHERE iec_no LIKE '%$iec%' AND created_at >= TO_TIMESTAMP('$threeDaysAgo 00:00:00', 'YYYY-MM-DD HH24:MI:SS')";
       $bill_of_entry_summary_query = "SELECT * FROM bill_of_entry_summary WHERE iec_no LIKE '%$iec%'  and created_at >= '$threeDaysAgo'";
...
public function boe_file_status()
{
      ...
      // Load secondary database
        ...
        for ($page = 0; $page < $pages; $page++)
        {
            ...
            {
                $iec = $user['iec_no'];
               $boe_file_status_query = "SELECT * FROM boe_file_status WHERE user_iec_no LIKE '%$iec%' AND created_at >= '$threeDaysAgo'";
...
public function boe_delete_logs()
{
 ...
 as total FROM lucrative_users WHERE role = 'admin'")->row()->total;
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            {
                 $iec = $user['iec_no'];
                 $boe_delete_logs_query="SELECT *  FROM boe_delete_logs WHERE iec_no LIKE '%$iec%'  and created_at >= '$threeDaysAgo'";
...
WHERE ship_bill_summary.iec LIKE '%$iec%' and ship_bill_summary.created_at >= '$threeDaysAgo'";
          ...
          $threeDaysAgo = date('Y-m-d', strtotime('-3 days'));
           ...
           for ($page = 0; $page < $pages; $page++)
           {
            ...
            foreach ($admin_users as $user) 
            {
                ...
                LEFT JOIN invoice_and_valuation_details ON invoice_and_valuation_details.invoice_id = duties_and_additional_details.invoice_id Where bill_of_entry_summary.iec_no Like '%$iec' and bill_of_entry_summary.created_at >= '$threeDaysAgo'  ";
 ...
 private function isDuplicateEntry_dynamic_duties_and_additional_details($detail, $users_id)
{
    try 
    {
           ...
           $result = $db_secondary->query($duplicate_query)->row_array();
             if ($result['num_rows'] > 0) {  echo "=====================";echo $users_id;
            // Delete existing duplicate entries based on the criteria
       echo     $delete_query = "DELETE FROM duties_and_additional_details 
                             WHERE duties_id = '{$detail['duties_id']}' 
                             AND boe_id = '{$detail['boe_id']}' AND invoice_id = '{$detail['invoice_id']}'";
...
public function cb_file_status()
{
    ...
    $threeDaysAgo = date('Y-m-d', strtotime('-3 days'));
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            foreach ($admin_users as $user) {
                 $iec = $user['iec_no'];
                $bill_of_entry_summary_query = "SELECT * FROM cb_file_status where user_iec_no Like '%$iec' AND created_at >= '$threeDaysAgo'";
...
public function courier_bill_summary()
{
      ...
      // Load secondary database
        ...
        $pages = ceil($totalUsers / $perPage); $threeDaysAgo = date('Y-m-d', strtotime('-3 days'));
 // Calculate the date 3 days ago
    $threeDaysAgo = date('Y-m-d', strtotime('-365 days'));
        for ($page = 0; $page < $pages; $page++) {
            $offset = $page * $perPage;
            $adminUsersQuery = "SELECT lucrative_users_id, iec_no FROM lucrative_users WHERE role = 'admin' order by lucrative_users_id ASC LIMIT $perPage OFFSET $offset";
            $admin_users = $this->db->query($adminUsersQuery)->result_array();
    ...
    $threeDaysAgo = date('Y-m-d', strtotime('-3 days'));
        for ($page = 0; $page < $pages; $page++) {
            ...
            foreach ($admin_users as $user) {
                 ...
                 LEFT JOIN cb_file_status ON courier_bill_summary.cb_file_status_id =cb_file_status.cb_file_status_id Where cb_file_status.user_iec_no Like '%$iec'  AND courier_bill_notification_used_for_items.created_at >= '$threeDaysAgo'";
      ...
      // Load secondary database
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            { 
                 ...
                 LEFT JOIN cb_file_status ON cb_file_status.cb_file_status_id=courier_bill_summary.cb_file_status_id where cb_file_status.user_iec_no Like '%$iec'  AND courier_bill_manifest_details.created_at >= '$threeDaysAgo'";
      ...
      // Load secondary database
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            foreach ($admin_users as $user) {
                ...
                LEFT JOIN cb_file_status ON courier_bill_summary.cb_file_status_id = cb_file_status.cb_file_status_id  Where cb_file_status.user_iec_no Like '%$iec'  AND courier_bill_items_details.created_at >= '$threeDaysAgo'";
      ...
      // Load secondary database
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            foreach ($admin_users as $user)
            {
                 ...
                 LEFT JOIN cb_file_status ON courier_bill_summary.cb_file_status_id = cb_file_status.cb_file_status_id  Where cb_file_status.user_iec_no Like '%$iec' AND courier_bill_invoice_details.created_at >= '$threeDaysAgo'";
      ...
      // Load secondary database
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            foreach ($admin_users as $user) {
                ...
                LEFT JOIN cb_file_status ON courier_bill_summary.cb_file_status_id = cb_file_status.cb_file_status_id  Where cb_file_status.user_iec_no Like '%$iec'  AND courier_bill_igm_details.created_at >= '$threeDaysAgo'";
 ...
 as total FROM lucrative_users WHERE role = 'admin'")->row()->total;
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            foreach ($admin_users as $user) 
            {
               ...
               join cb_file_status as cfs on cfs.cb_file_status_id = cbs.cb_file_status_id Where cfs.user_iec_no Like '%$iec' AND courier_bill_duty_details.created_at >= '$threeDaysAgo'";
...
public function courier_bill_container_details()
{
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            foreach ($admin_users as $user)
            {
                ...
                WHERE cfs.user_iec_no LIKE '%$iec'  AND cbc.created_at >= '$threeDaysAgo'";  
...
public function courier_bill_bond_details()
{
        ...
        for ($page = 0; $page < $pages; $page++)
        {
            ...
            foreach ($admin_users as $user) 
            {
                ...
                LEFT JOIN cb_file_status cfs ON cfs.cb_file_status_id = cbs.cb_file_status_id 
                WHERE cfs.user_iec_no LIKE '%$iec' AND cbd.created_at >= '$threeDaysAgo'"; 
 ...
 public function bill_bond_details()
    {
      // Load secondary database
        ...
        FROM bill_bond_details LEFT JOIN bill_of_entry_summary ON bill_of_entry_summary.boe_id = bill_bond_details.boe_id and bill_of_entry_summary.iec_no LIKE '%$iec%'  AND bill_bond_details.created_at >= '$threeDaysAgo'";
 ...
 $threeDaysAgo = date('Y-m-d', strtotime('-365 days'));
        // Fetch licence details efficiently
         $licenceQuery = "SELECT ald.*, sbs.sbs_id, sbs.sb_no, sbs.iec, id.invoice_id, id.item_id
            ...
            WHERE sbs.iec LIKE '%$iec%'  AND ald.created_at >= '$threeDaysAgo'";
    ...
    public function bill_payment_details()
{
      ...
      // Load secondary database
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            WHERE bill_of_entry_summary.iec_no  LIKE '%$iec%' and bill_payment_details.created_at >= '$threeDaysAgo'";
...
public function bill_container_details()
{
     ...
     // Load secondary database
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            WHERE bill_of_entry_summary.iec_no  LIKE '%$iec%' and bill_of_entry_summary.created_at >= '$threeDaysAgo'";
...
public function bill_licence_details()
{
    ...
    WHERE n1.iec_no  LIKE '%$iec%' and bill_licence_details.created_at >= '$threeDaysAgo'";
...
foreach ($valid_query_details as $detail) {
    try {
        ...
        $isDuplicate = $this->isDuplicateEntry_dynamic_bill_licence_details($detail, $users_id);
        ...
        if (!$isDuplicate) {
            ...
            // Detail is a duplicate, log it as informational and continue to next detail
           // log_message('info', 'Duplicate entry found for ID ' . $detail['id'] . '. Skipping this record.');
        }
    } catch (Exception $e) {
        // Handle database error
        if ($e->getCode() == '23000' && strpos($e->getMessage(), 'Duplicate entry') !== false) {
            // Duplicate entry error detected, log it as informational and continue
...
// Continue script execution after processing all details
...
private function isDuplicateEntry_dynamic_bill_licence_details($detail, $users_id)
{
    try { 
        $db_secondary = $this->load_secondary_database($users_id);
          ...
          if ($result['num_rows'] > 0) {  echo "=====================";echo $users_id;
            // Delete existing duplicate entries based on the criteria
       echo     $delete_query = "DELETE FROM bill_licence_details   where licence_id ='{$detail['licence_id']}'  and lic_slno='{$detail['lic_slno']}'  and lic_no='{$detail['lic_no']}' ";
            ...
            log_message('error', 'Query execution failed in isDuplicateEntry_dynamic_bill_licence_details');
        }
    } catch (Exception $e) {
        // Handle database error
        log_message('error', 'Database Error in isDuplicateEntry_dynamic: ' . $e->getMessage());
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            foreach ($admin_users as $user) {
                ...
                $bill_manifest_details_query = "SELECT bill_manifest_details.*, bill_of_entry_summary.boe_id as boeid, bill_of_entry_summary.be_no, bill_of_entry_summary.iec_no 
                                        ...
                                        WHERE bill_of_entry_summary.iec_no LIKE '%$iec%' and bill_manifest_details.created_at >= '$threeDaysAgo'";
...
public function challan_details(){    
    ...
    JOIN ship_bill_summary ON ship_bill_summary.sbs_id=challan_details.sbs_id  Where ship_bill_summary.iec LIKE '%$iec%' and challan_details.created_at >= '$threeDaysAgo' ";
           ...
           //echo '<pre>'; print_r( $admin_users);
            foreach ($admin_users as $user) {
              ...
              echo  $bill_manifest_details_query = "SELECT CONCAT(n1.sb_no,'-',invoice_summary.s_no_inv, '-', item_details.item_s_no) as reference_code, sb_no, sb_date, iec_br, iec, inv_sno, item_sno, item_details.hs_cd, item_details.description, dbk_sno, qty_wt, value, dbk_amt, stalev, cenlev, drawback_details.* 
                                    ...
                                    WHERE n1.iec LIKE '%$iec%'  and drawback_details.created_at >= '$threeDaysAgo'";
...
public function item_details(){    
     // Load secondary database
        ...
        JOIN invoice_summary ON invoice_summary.invoice_id=item_details.invoice_id 
        JOIN ship_bill_summary ON ship_bill_summary.sbs_id=invoice_summary.sbs_id WHERE ship_bill_summary.iec LIKE '%$iec%' and item_details.created_at >= '$threeDaysAgo'";
    ...
    for ($page = 0; $page < $pages; $page++) {
        ...
        foreach ($admin_users as $user) {
            ...
            $invoice_and_valuation_details_query = "SELECT invoice_and_valuation_details.*, bill_of_entry_summary.iec_no 
                                                    ...
                                                    WHERE bill_of_entry_summary.iec_no LIKE '%$iec%' and invoice_and_valuation_details.created_at >= '$threeDaysAgo' ";
...
private function isDuplicateEntry_dynamic_invoice_and_valuation_details($detail, $users_id)
{
    try {
        ...
        $duplicate_query = "SELECT COUNT(*) AS num_rows 
                            FROM invoice_and_valuation_details  
                            WHERE invoice_id= '{$detail['invoice_id']}' and s_no = '{$detail['s_no']}' AND invoice_no = '{$detail['invoice_no']}' AND invoice_date = '{$detail['invoice_date']}'";

        
             $result = $db_secondary->query($duplicate_query)->row_array();
       if ($result['num_rows'] > 0) {  echo "=====================";echo $users_id;
            // Delete existing duplicate entries based on the criteria
       echo     $delete_query = "DELETE FROM invoice_and_valuation_details 
                             WHERE invoice_id= '{$detail['invoice_id']}' and s_no = '{$detail['s_no']}' AND invoice_no = '{$detail['invoice_no']}'
                             AND invoice_date = '{$detail['invoice_date']}'";
...
public function jobbing_details(){    
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            foreach ($admin_users as $user) {
                 $iec = $user['iec_no'];
                $jobbing_details_query = "SELECT jobbing_details.*,ship_bill_summary.iec,ship_bill_summary.sbs_id ,ship_bill_summary.sb_no FROM jobbing_details JOIN ship_bill_summary ON ship_bill_summary.sbs_id=jobbing_details.sbs_id Where ship_bill_summary.iec LIKE '%$iec%' and jobbing_details.created_at >= '$threeDaysAgo'";
...
public function ship_bill_summary()
{
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            foreach ($admin_users as $user) {
                 $iec = $user['iec_no'];
              //echo  $jobbing_details_query = " SELECT * FROM ship_bill_summary Where iec LIKE '%$iec%' and ship_bill_summary.created_at >= '$threeDaysAgo'";
...
/********************************************************************* equipment_details************************************************************************************/
 ...
 public function equipment_details(){    
  ...
  LEFT JOIN ship_bill_summary ON ship_bill_summary.sbs_id=equipment_details.sbs_id  where ship_bill_summary.iec Like '%$iec%' and equipment_details.created_at >= '$threeDaysAgo'";
    ...
    // Load secondary database
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            foreach ($admin_users as $user) {
              ...
              ship_bill_summary.sbs_id=invoice_summary.sbs_id WHERE ship_bill_summary.iec LIKE '%$iec'  and item_details.created_at >= '$threeDaysAgo'";
...
private function isDuplicateEntry_dynamic_invoice_summery($detail, $users_id)
{
    try {
        $db_secondary = $this->load_secondary_database($users_id);
     echo   $duplicate_query = "SELECT COUNT(*) AS num_rows   FROM invoice_summary 
...
foreach ($admin_users as $user) {
     $iec = $user['iec_no'];
    $bill_of_entry_summary_query = "SELECT * FROM sb_file_status Where user_iec_no Like '%$iec' and created_at >= '$threeDaysAgo'";
