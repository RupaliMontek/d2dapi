...
class Admin_cron_local_server extends CI_Controller
{
...
public function boe_delete_logs()
{
 ...
 as total FROM lucrative_users WHERE role = 'admin'")->row()->total;
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            {
                 $iec = $user['iec_no'];
                 $boe_delete_logs_query="SELECT *  FROM boe_delete_logs WHERE iec_no LIKE '%$iec%'";
 ...
 private function isDuplicateEntry_dynamic_boe_delete_logs($detail, $users_id)
{
    try {
            ...
            FROM boe_delete_logs be_no='{$detail['be_no']}'  and iec_no='{$detail['iec_no']}' and be_date='{$detail['be_date']}' and boe_delete_logs_id ='{$detail['boe_delete_logs_id']}'";
            // Execute the duplicate query and fetch the result
            $result = $db_secondary->query($duplicate_query)->row_array();
            // Close secondary database connection
            $db_secondary->close();
...
public function third_party_details()
{
      ...
      // Load secondary database
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            foreach ($admin_users as $user) {
                $iec = $user['iec_no'];
                $third_party_details_query = "SELECT  CONCAT(n1.sb_no,'-',invoice_summary.s_no_inv, '-', item_details.item_s_no) as reference_code, 
                ...
                JOIN invoice_summary ON invoice_summary.sbs_id = n1.sbs_id  Where n1.iec Like '%$iec'
...
public function duties_and_additional_details()
{
      ...
      // Load secondary database
        ...
        // Fetch admin users with paginated queries to handle large datasets
          $perPage = 50; // Adjust based on memory and performance testing
          $totalUsers = $this->db->query("SELECT COUNT(*)
          as total FROM lucrative_users WHERE role = 'admin'and lucrative_users_id='10'")->row()->total;
          $pages = ceil($totalUsers / $perPage);

           for ($page = 0; $page < $pages; $page++)
           {
            $offset = $page * $perPage;
            $adminUsersQuery = "SELECT lucrative_users_id, iec_no FROM lucrative_users WHERE role = 'admin' and lucrative_users_id='10' order by lucrative_users_id DESC LIMIT $perPage OFFSET $offset";
            $admin_users = $this->db->query($adminUsersQuery)->result_array();
...
public function sb_file_status()
{
      ...
      // Load secondary database
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            foreach ($admin_users as $user) {
                 $iec = $user['iec_no'];
                $bill_of_entry_summary_query = "SELECT * FROM sb_file_status Where user_iec_no Like '%$iec'";
...
public function cb_file_status()
{
      ...
      // Load secondary database
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            foreach ($admin_users as $user) {
                 $iec = $user['iec_no'];
                $bill_of_entry_summary_query = "SELECT * FROM cb_file_status where user_iec_no Like '%$iec'";
...
public function courier_bill_summary()
{
      ...
      // Load secondary database
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            {
                 $iec = $user['iec_no'];
                 $bill_of_entry_summary_query = "SELECT courier_bill_summary.*,cb_file_status.user_iec_no ,cb_file_status.cb_file_status_id FROM courier_bill_summary LEFT JOIN cb_file_status ON courier_bill_summary.cb_file_status_id =cb_file_status.cb_file_status_id   Where cb_file_status.user_iec_no Like '%$iec'";
     ...
     private function isDuplicateEntry_dynamic_courier_bill_notification_used_for_items($detail, $users_id)
    {
        try {
                ...
                $duplicate_query = "SELECT COUNT(*) AS num_rows  FROM courier_bill_notification_used_for_items LEFT JOIN courier_bill_items_details ON courier_bill_notification_used_for_items.items_detail_id=courier_bill_items_details.items_detail_id 
                 LEFT JOIN courier_bill_summary ON courier_bill_summary.courier_bill_of_entry_id=courier_bill_items_details.courier_bill_of_entry_id 
                 LEFT JOIN cb_file_status ON courier_bill_summary.cb_file_status_id =cb_file_status.cb_file_status_id   where igm_details_id ='{$detail['igm_details_id']}'  and courier_bill_notification_used_for_items.courier_bill_of_entry_id ='{$detail['courier_bill_of_entry_id']}'";
      ...
      // Load secondary database
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            { 
                 ...
                 LEFT JOIN cb_file_status ON cb_file_status.cb_file_status_id=courier_bill_summary.cb_file_status_id where cb_file_status.user_iec_no Like '%$iec'";
 ...
 as total FROM lucrative_users WHERE role = 'admin'")->row()->total;
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            foreach ($admin_users as $user) 
            {
               ...
               join cb_file_status as cfs on cfs.cb_file_status_id = cbs.cb_file_status_id Where cfs.user_iec_no Like '%$iec'";
...
public function courier_bill_container_details()
{
        ...
        for ($page = 0; $page < $pages; $page++) {
            ...
            foreach ($admin_users as $user)
            {
                ...
                WHERE cfs.user_iec_no LIKE '%$iec'";  
...
public function filterDuplicateEntries_dynamic_bill_container_details($valid_query_details, $users_id,$unwanted_fields)
{
     $filtered_details = [];
    foreach ($valid_query_details as $detail) 
    {
...
public function isDuplicateEntry_dynamic_bill_container_details($detail, $users_id)
{
    try {
                $db_secondary = $this->load_secondary_database($users_id);
                $duplicate_query = 
...
public function aa_dfia_licence_details()
{
    ...
    $PostgearSQL_admin_users = "SELECT lucrative_users_id, iec_no FROM lucrative_users WHERE role = 'admin' ORDER BY lucrative_users_id DESC
";
...
private function filterDuplicateEntries($licence_details, $users_id)
{
    ...
    foreach ($licence_details as $detail) {
        ...
        $result = $this->db->query($duplicate_check_query)->row_array();
...
public function bill_of_entry_summary_old()
{
    // Load PostgearSQL database
  $this->load->database('second');
...
public function boe_delete_logs_old()
{
    // Load PostgearSQL database
    $this->load->database('second');
...
public function boe_file_status11()
{
    // Load PostgearSQL database
    $this->load->database('second');
...
public function bill_manifest_details_old()
{
    // Load PostgearSQL database
    $this->load->database('second');
...
public function cb_file_status_old()
{
    // Load PostgearSQL database
    $this->load->database('second');
...
public function courier_bill_bond_detailsy()
{
    // Load PostgearSQL database
    $this->load->database('second');
...
public function courier_bill_container_details_old()
{
...
public function courier_bill_duty_details_old()
{
    // Load the primary database
    $db_primary = $this->load->database('second', TRUE);
...
public function courier_bill_igm_details_old(){    
...
public function courier_bill_invoice_details_old()
{
    // Load the primary database
    $db_primary = $this->load->database('second', TRUE);
...
public function courier_bill_items_details_old(){    
...
public function courier_bill_manifest_details_old(){    
        /******************************************************************Start courier_bill_container_details***************************************************************************************/
...
public function courier_bill_notification_used_for_items_old(){    
        /******************************************************************Start courier_bill_container_details***************************************************************************************/
...
public function courier_bill_payment_details_old(){    
...
public function courier_bill_procurment_details_old(){    
...
public function courier_bill_summary_old(){    
...
public function invoice_and_valuation_details_olddd() {    
    // Load the primary database
    $db_primary = $this->load->database('second', TRUE);
...
public function duties_and_additional_details_old(){    
    ...
    // Process each admin user
        foreach ($batch2 as $str_duties_and_additional_details) {
          ...
          //  if (get_magic_quotes_gpc()) {
            ...
            $db_secondary = $this->load_secondary_database($lucrative_users_id);
            // Load the secondary database manually based on user id
       		/********************checking dupliacte entries d2d***********************/
            $duplicate_check_query = $db_secondary->query("SELECT COUNT(*) AS num_rows  FROM duties_and_additional_details 
            LEFT JOIN bill_of_entry_summary ON duties_and_additional_details.boe_id = bill_of_entry_summary.boe_id
            ...
            $iecwise1_users = $duplicate_check_query->row_array();
            $iecwise_data1_users = array();
            if ($iecwise1_users['num_rows']> 0)
            {  
                continue;
...
if(empty($caidc_cvd_edc_notn_sno))
{
    $caidc_cvd_edc_notn_sno = 0;
}    
...
$created_at."')";  
           ...
           $copy_insert_duties_and_additional_details = $db_secondary->query($sql_insert_duties_and_additional_details);
...
public function sb_file_status111111(){    
...
VALUES('" .
 ...
 public function third_party_details111(){    
        /******************************************************************Start sb_file_status***************************************************************************************/
            // Load the primary database
    $db_primary = $this->load->database('second', TRUE);
    ...
    /*****************************************Create Table bill_bond_details***************************************************************/
           ...
           /*****************************************Create Table bill_container_details***************************************************************/
               $db2->query("CREATE TABLE `bill_container_details` (
                          `container_details_id` int(20) NOT NULL,
                          `boe_id` int(20) NOT NULL,
       ...
       /*****************************************Create Table bill_manifest_details***************************************************************/
            ...
            /*****************************************Create Table challan_details***************************************************************/
             ...
             /*****************************************Create Table courier_bill_container_details***************************************************************/
                        ...
                        $db2->query("CREATE TABLE `courier_bill_container_details` (
                                       `courier_bill_of_entry_id` int(20) NOT NULL,
                                      `container_details_id` int(20) DEFAULT NULL PRIMARY KEY,
           ...
           /*****************************************Create Table courier_bill_bond_details***************************************************************/
                        ...
                        $db2->query("CREATE TABLE `courier_bill_bond_details` (
                                      `courier_bill_of_entry_id` int(20)  NOT NULL,
                                      `bond_details_id` int(20) DEFAULT NULL  PRIMARY KEY,
         ...
         /*****************************************Create Table courier_bill_duty_details***************************************************************/
                        ...
                        $db2->query("CREATE TABLE `courier_bill_duty_details` (
                                      `items_detail_id` int(20) NOT NULL,
                                      `duty_details_id` int(20) DEFAULT NULL PRIMARY KEY,
          ...
          /*****************************************Create Table courier_bill_invoice_details***************************************************************/
                        ...
                        $db2->query("CREATE TABLE `courier_bill_invoice_details` (
                                    ...
                                    `supplier_details_address` varchar(256) DEFAULT NULL,
        ...
        /*****************************************Create Table courier_bill_notification_used_for_items***************************************************************/
               $db2->query("CREATE TABLE `courier_bill_notification_used_for_items` (
                          `items_detail_id` int(20) NOT NULL,
                          `item_notification_id` int(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
            ...
            /*****************************************Create Table duties_and_additional_details***************************************************************/
               $db2->query("CREATE TABLE `duties_and_additional_details` (
                                     `boe_id` int(20) DEFAULT NULL,
                                      ...
                                      `load_date` text DEFAULT NULL,
                                      `pf_` text DEFAULT NULL,
                                      `beno` text DEFAULT NULL,
                                      `bedate` text DEFAULT NULL,
...
public function register_iec_user_old(){    
  ...
  `cb_file_status_id` int4 PRIMARY KEY NOT NULL,
  `pdf_filepath` varchar(255),
  `pdf_filename` varchar(255),
  `user_iec_no` varchar(255),
  ...
  `courier_bill_of_entry_id` integer PRIMARY KEY NOT NULL,
  ...
  `manifest_details_id` integer PRIMARY KEY NOT NULL,
  ...
  `courier_bill_of_entry_id` integer,
  `procurment_details_id` integer PRIMARY KEY NOT NULL,
 ...
 $this->db->query("CREATE TABLE `invoice_details` (
  `courier_bill_of_entry_id` integer,
  `invoice_detail_id` integer PRIMARY KEY NOT NULL,
  `invoice_number` text,
  ...
  `courier_bill_of_entry_id` integer,
  `igm_details_id` integer PRIMARY KEY NOT NULL,
  ...
  `courier_bill_of_entry_id` integer,
  `container_details_id` integer PRIMARY KEY NOT NULL,
  ...
  `courier_bill_of_entry_id` integer,
  `bond_details_id` integer PRIMARY KEY NOT NULL,
  ...
  `courier_bill_of_entry_id` integer,
  `items_detail_id` integer PRIMARY KEY NOT NULL,
  ...
  `item_notification_id` integer PRIMARY KEY NOT NULL,
  ...
  `items_detail_id` integer,
  `duty_details_id` integer PRIMARY KEY NOT NULL,
