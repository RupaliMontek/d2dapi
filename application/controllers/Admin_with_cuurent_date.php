...
class Admin extends CI_Controller {
	...
	/******************************************************************Start aa_dfia_licence_details***************************************************************************************/
           ...
           $query = "SELECT aa_dfia_licence_details.*, ship_bill_summary.sbs_id, ship_bill_summary.iec,item_details.invoice_id,item_details.item_id FROM aa_dfia_licence_details LEFT JOIN item_details ON aa_dfia_licence_details.item_id = item_details.item_id LEFT JOIN invoice_summary ON invoice_summary.invoice_id = item_details.invoice_id LEFT JOIN ship_bill_summary ON invoice_summary.sbs_id = ship_bill_summary.sbs_id WHERE date(aa_dfia_licence_details.created_at) = CURRENT_DATE LIMIT 5";
...
$copy_insert_courier_bill_igm_details = $db1_insert_courier_bill_igm_details->query($sql_insert_courier_bill_igm_details);    
	...
	/******************************************************************Start courier_bill_invoice_details***************************************************************************************/
            ...
            $query_courier_bill_invoice_details = "SELECT courier_bill_invoice_details.*,cb_file_status.user_iec_no ,cb_file_status.cb_file_status_id FROM courier_bill_invoice_details LEFT JOIN cb_file_status ON courier_bill_invoice_details.courier_bill_of_entry_id = cb_file_status.cb_file_status_id WHERE date(courier_bill_invoice_details.created_at) = CURRENT_DATE LIMIT 5";
